    ...
    <>
      ...
      <span className="flex items-center gap-2">
        ...
        <select
          ...
          className="bg-slate-800 border border-slate-600 rounded px-2 py-0.5 text-[10px] text-white focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 cursor-pointer"
        >
          <option value="current">Current</option>
          {versions.map((version) => (
            <option key={version.id} value={version.id}>
              {formatDate(version.createdAt)} ({version.provider}/{version.model || "unknown"})
            </option>
          ))}
        </select>
      </span>
