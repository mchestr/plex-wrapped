...
import { saveRadarr } from "@/actions/setup"
import { StyledInput } from "@/components/ui/styled-input"
import { type RadarrInput } from "@/lib/validations/radarr"
...
/** Check if all required fields are populated for Radarr form */
function isFormComplete(data: RadarrInput): boolean {
  return !!(data.name?.trim() && data.url?.trim() && data.apiKey?.trim())
}

export function RadarrForm({ onComplete, onBack }: RadarrFormProps) {
  const [isPending, startTransition] = useTransition()
  const [isSuccess, setIsSuccess] = useState(false)
  const [errors, setErrors] = useState<Record<string, string>>({})
  const [formData, setFormData] = useState<RadarrInput>({
    name: "",
    url: "",
    apiKey: "",
    publicUrl: "",
  ...
  useEffect(() => {
    ...
    getDevDefaults().then((defaults) => {
      ...
      if (defaults.radarr) {
        setFormData((prev) => ({
          name: defaults.radarr?.name ?? prev.name,
          url: defaults.radarr?.url ?? prev.url,
          apiKey: defaults.radarr?.apiKey ?? prev.apiKey,
          publicUrl: defaults.radarr?.publicUrl ?? prev.publicUrl,
        }))
      }
    })
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await saveRadarr(formData)
      if (result.success) {
        setIsSuccess(true)
        onComplete()
      } else {
        setErrors({ submit: result.error || "Failed to save Radarr configuration" })
      }
    })
  }
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-300 mb-6">
          Enter your Radarr server connection details. You can find your API key
          in Radarr settings under General → Security.
        </p>
      </div>
      ...
      <div>
        ...
        <StyledInput
          ...
          value={formData.url}
          onChange={handleChange}
          placeholder="http://radarr.example.com:7878 or http://192.168.1.100:7878"
          size="md"
          className="mt-1"
          error={!!errors.url}
        />
      ...
      <div>
        ...
        />
        <p className="mt-1 text-xs text-slate-400">
          The public URL for Radarr (for links/redirects)
        </p>
        {errors.publicUrl && (
